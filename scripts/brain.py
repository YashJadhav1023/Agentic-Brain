...
def cmd_claude(args: argparse.Namespace) -> None:
    """Manage Claude Code accounts, each isolated by its own CLAUDE_CONFIG_DIR.

    Adding an account never asks for a subscription credential. The account's
    profile directory is created, and the human is told the exact command to run
    against the official CLI to log in. That keeps the OAuth exchange with the
    first-party client, which is both the supported path and the one that cannot
    get a contributor's account banned for client impersonation.
    """
    ...
    action = args.claude_command

    def _adapters() -> list[ClaudeAdapter]:
        from integrations.pxpipe import PxpipeManager

        base_url = (
            PxpipeManager.from_config().base_url
            if PxpipeManager.enabled_in_config()
            else None
        )
        return ClaudeAdapter.load_from_config(pxpipe_base_url=base_url)

    if action == "list":
        adapters = _adapters()
        if getattr(args, "json", False):
            print(json.dumps([a.describe() for a in adapters], indent=2))
            return
        if not adapters:
            print("No Claude accounts configured. Add one with: brain claude add --name account-1")
            return
        print("\nACCOUNT                AUTH           PLAN        PXPIPE  STATUS")
        print("-" * 78)
        for adapter in adapters:
            profile = adapter.profile_status() if adapter.auth_mode is ClaudeAuthMode.SUBSCRIPTION else None
            healthy, reason = adapter.health()
            plan = (profile.plan if profile else "-") or "-"
            px = "on" if adapter.pxpipe_base_url else "off"
            state = "READY" if healthy else "NEEDS LOGIN" if profile and not profile.logged_in else "OFFLINE"
            print(f"{adapter.agent_id:<22} {adapter.auth_mode.value:<14} {plan:<11} {px:<7} {state}")
            if not healthy:
                print(f"  -> {reason}")
        return

    if action == "add":
        name = args.name or input("Account name (e.g. account-2): ").strip()
        if not name:
            print("Error: account name cannot be empty.", file=sys.stderr)
            sys.exit(1)
        agent_id = name if name.startswith("claude-") else f"claude-{name}"

        if get_account("claude", agent_id):
            print(f"Error: account '{agent_id}' already exists. Remove it first or pick another name.", file=sys.stderr)
            sys.exit(1)

        auth_mode = ClaudeAuthMode.parse(args.auth_mode)
        meta = get_provider_meta("claude")
        profile_root = meta.get("profile_root", "~/.mission-control/claude")
        config_dir = args.config_dir or f"{profile_root}/{name}"

        cred_ref = ""
        if auth_mode is not ClaudeAuthMode.SUBSCRIPTION:
            # A secret is only ever taken through a hidden prompt and handed
            # straight to the CredentialManager; it is never written to config.
            from providers.registry.credential_manager import get_credential_manager

            import getpass

            label = "OAuth token from `claude setup-token`" if auth_mode is ClaudeAuthMode.OAUTH_TOKEN else "Anthropic API key"
            secret = getpass.getpass(f"Enter {label} (input hidden, leave blank to use the environment): ").strip()
            if secret:
                cred_ref = f"secret://mission-control/claude/{name}"
                get_credential_manager().store(cred_ref, secret)
                print(f"Stored credential under reference: {cred_ref}")

        try:
            created = ensure_profile_dir(config_dir)
        ...
        account_data = {
            ...
            "provider_id": "claude",
            "enabled": True,
            "description": args.description or f"Claude account {name}",
            "auth_mode": auth_mode.value,
            "authentication_type": auth_mode.value,
            "credential_reference": cred_ref,
            "config_dir": str(config_dir),
            "use_pxpipe": bool(args.use_pxpipe),
            "execution": {
                "command": meta.get("command", "~/.local/bin/claude"),
                "output_format": "json",
                "dangerously_skip_permissions": False,
                "default_timeout_seconds": int(args.timeout),
            },
            "capabilities": [c.strip() for c in args.capabilities.split(",")] if args.capabilities else [
                "deep-reasoning", "architecture", "code-review",
                "component-refactoring", "editor-refactoring",
                "test-scaffolding", "documentation",
            ],
            "models": [m.strip() for m in args.models.split(",")] if args.models else ["sonnet", "opus", "haiku"],
            "default_model": args.default_model or "sonnet",
        }
        add_account_config("claude", agent_id, account_data)

        print(f"\nAdded Claude account '{agent_id}'.")
        print(f"  profile dir : {created} (mode 0700)")
        print(f"  auth mode   : {auth_mode.value}")
        print(f"  pxpipe      : {'on' if args.use_pxpipe else 'off'}")
        if auth_mode is ClaudeAuthMode.SUBSCRIPTION:
            status = inspect_profile(config_dir)
            if status.logged_in:
                print(f"  login       : already logged in ({status.plan})")
            else:
                print("\nThis account is not logged in yet. Run this once, interactively:")
                print(f"\n  {login_command(config_dir, meta.get('command', 'claude'))}\n")
                print("The credential stays in that directory, owned by the official CLI.")
                print("The brain never reads, copies or stores it.")
        print(f"\nVerify with: brain claude status {agent_id}")
        return

    if action == "login":
        account = get_account("claude", args.agent_id)
        if not account:
            print(f"Account not found: {args.agent_id}", file=sys.stderr)
            sys.exit(1)
        meta = get_provider_meta("claude")
        config_dir = account.raw.get("config_dir")
        executable = account.raw.get("execution", {}).get("command") or meta.get("command", "claude")
        command = login_command(config_dir, executable)
        status = inspect_profile(config_dir)
        if status.logged_in and not args.force:
            print(f"{args.agent_id} is already logged in ({status.plan}).")
            print(f"Re-authenticate anyway with --force, or log out with:\n  {logout_command(config_dir, executable)}")
            return
        print(f"\nLog in to '{args.agent_id}' by running this interactively:\n")
        print(f"  {command}\n")
        print("Why this is not run for you: the OAuth exchange belongs to the official")
        print("Claude CLI. Driving it from here would require impersonating its client")
        print("id, which risks your account. The brain only reads login metadata.")
        return

    if action == "status":
        adapters = _adapters()
        if args.agent_id:
            adapters = [a for a in adapters if a.agent_id == args.agent_id]
            if not adapters:
                print(f"Account not found or disabled: {args.agent_id}", file=sys.stderr)
                sys.exit(1)
        payload = [a.describe() for a in adapters]
        if getattr(args, "json", False):
            print(json.dumps(payload, indent=2))
            return
        for item in payload:
            print(f"\n=== {item['agent_id']} ===")
            print(f"Auth mode        : {item['auth_mode']}")
            print(f"Profile dir      : {item['profile']}")
            print(f"pxpipe           : {item['pxpipe_base_url'] or 'off'}")
            print(f"Models           : {', '.join(item['models'])}")
            print(f"Healthy          : {item['health']['healthy']} ({item['health']['reason']})")
            sub = item.get("subscription")
            if sub:
                print(f"Plan             : {sub['plan'] or 'unknown'} (tier {sub['rate_limit_tier'] or 'unknown'})")
                if sub["seconds_until_expiry"] is not None:
                    print(f"Credential expiry: in {sub['seconds_until_expiry']}s"
                          + (" -- EXPIRING SOON" if sub["expiring_soon"] else ""))
                if sub["world_readable"]:
                    print("WARNING          : credential file is readable by other local users")
        return
    ...
    if action == "remove":
        ...
        if not account:
            print(f"Account not found: {args.agent_id}", file=sys.stderr)
            sys.exit(1)
        config_dir = resolve_config_dir(account.raw.get("config_dir"))
        remove_account_config("claude", args.agent_id)
        print(f"Removed account '{args.agent_id}' from configuration.")
        # The profile directory holds a live credential. Deleting it is a
        # separate, explicit act: an accidental `remove` must not silently
        # revoke a session or destroy another account's data.
        print(f"Its profile directory was left in place: {config_dir}")
        print(f"To revoke the session, run: {logout_command(config_dir)}")
        return

    print(f"Unknown claude subcommand: {action}", file=sys.stderr)
    sys.exit(1)


def cmd_pxpipe(args: argparse.Namespace) -> None:
    """Control the local pxpipe token-compression proxy.

    pxpipe rewrites bulky request context as PNG pages so token-dense context
    costs image tokens instead of text tokens. It is loopback-only and holds no
    credential: an agent's own CLI supplies auth, which pxpipe forwards.
    """
    from integrations.pxpipe import PxpipeManager

    manager = PxpipeManager.from_config()
    action = args.pxpipe_command

    if action == "status":
        status = manager.status()
        enabled = PxpipeManager.enabled_in_config()
        if getattr(args, "json", False):
            print(json.dumps({**status.to_dict(), "enabled_in_config": enabled}, indent=2))
            return
        print(f"\nEnabled in config : {enabled}")
        print(f"Base URL          : {status.base_url}")
        print(f"Running           : {status.running} ({status.reason})")
        print(f"Install source    : {status.source}")
        print(f"PID file          : {manager.pid_file}")
        print(f"Event log         : {manager.events_file}")
        if not status.running:
            print("\nStart it with: brain pxpipe start")
        return
    ...
    if action == "start":
        ...
        if getattr(args, "json", False):
            ...
            verb = "running" if status.running else "FAILED to start"
            print(f"pxpipe {verb} at {status.base_url} ({status.reason})")
            if status.running:
                print(f"Dashboard: {status.base_url}/   Log: {manager.log_file}")
        if not status.running:
            sys.exit(1)
        return

    if action == "stop":
        ok, reason = manager.stop()
        print(reason)
        if not ok:
            sys.exit(1)
        return
    ...
    if action == "savings":
        report = manager.savings(limit=args.limit)
        if getattr(args, "json", False):
            print(json.dumps(report.to_dict(), indent=2))
            return
        print(f"\n=== pxpipe measured savings ===")
        print(f"Event log            : {report.events_file}")
        print(f"Rows                 : {report.total_rows}")
        print(f"Measured requests    : {report.measured_requests}")
        print(f"Unmeasured requests  : {report.unmeasured_requests} (no baseline probe; excluded)")
        print(f"Unbilled requests    : {report.unbilled_requests} (failed or unbilled upstream; excluded)")
        print(f"Compressed requests  : {report.compressed_requests}")
        print(f"Images emitted       : {report.images_emitted}")
        print(f"Baseline tokens      : {report.baseline_tokens:,}")
        print(f"Billed input tokens  : {report.billed_input_tokens:,}")
        print(f"Saved                : {report.saved_tokens:,} tokens ({report.saved_pct}%)")
        if report.measured_requests == 0:
            print("\nNo measured rows yet. Route an account through pxpipe (use_pxpipe: true)")
            print("and run a task; savings are measured per request against a free count_tokens probe.")
        for err in report.errors:
            print(f"note: {err}")
        return

    print(f"Unknown pxpipe subcommand: {action}", file=sys.stderr)
    sys.exit(1)
...
def main() -> None:
    ...
    # --- Claude Code accounts (subscription, oauth token, or API key) -------
    p_claude = sub.add_parser(
        "claude",
        help="Manage Claude Code accounts (subscription login, OAuth token, or API key)",
    )
    p_claude_sub = p_claude.add_subparsers(dest="claude_command", required=True)

    p_claude_list = p_claude_sub.add_parser("list", help="List configured Claude accounts")
    p_claude_list.add_argument("--json", action="store_true", help="Emit JSON output")

    p_claude_add = p_claude_sub.add_parser(
        "add",
        help="Add a Claude account (defaults to subscription: no secret is stored)",
    )
    p_claude_add.add_argument("--name", help="Account name, e.g. account-2")
    p_claude_add.add_argument(
        "--auth-mode",
        default="subscription",
        choices=["subscription", "oauth_token", "api_key"],
        help="subscription = Pro/Max seat logged in with the official CLI (default)",
    )
    p_claude_add.add_argument("--config-dir", help="Profile directory (default: <profile_root>/<name>)")
    p_claude_add.add_argument("--use-pxpipe", action="store_true", help="Route this account through the local pxpipe proxy")
    p_claude_add.add_argument("--models", help="Comma-separated model ids")
    p_claude_add.add_argument("--default-model", help="Default model id (default: sonnet)")
    p_claude_add.add_argument("--capabilities", help="Comma-separated capabilities")
    p_claude_add.add_argument("--description", help="Human description of this account")
    p_claude_add.add_argument("--timeout", type=int, default=300, help="Default execution timeout in seconds")

    p_claude_login = p_claude_sub.add_parser("login", help="Show the login command for an account")
    p_claude_login.add_argument("agent_id", help="Account id, e.g. claude-account-2")
    p_claude_login.add_argument("--force", action="store_true", help="Show it even if already logged in")

    p_claude_status = p_claude_sub.add_parser("status", help="Show login and health state, without exposing credentials")
    p_claude_status.add_argument("agent_id", nargs="?", help="Account id (default: all)")
    p_claude_status.add_argument("--json", action="store_true", help="Emit JSON output")

    p_claude_rm = p_claude_sub.add_parser("remove", help="Remove an account from configuration (keeps its profile dir)")
    p_claude_rm.add_argument("agent_id", help="Account id")

    # --- pxpipe token-compression proxy -------------------------------------
    p_px = sub.add_parser("pxpipe", help="Control the local pxpipe token-compression proxy")
    p_px_sub = p_px.add_subparsers(dest="pxpipe_command", required=True)

    p_px_status = p_px_sub.add_parser("status", help="Show proxy state and install source")
    p_px_status.add_argument("--json", action="store_true", help="Emit JSON output")

    p_px_start = p_px_sub.add_parser("start", help="Start the proxy on loopback and wait until it answers")
    p_px_start.add_argument("--json", action="store_true", help="Emit JSON output")

    p_px_sub.add_parser("stop", help="Stop a proxy the brain started")

    p_px_savings = p_px_sub.add_parser("savings", help="Measured token savings from the proxy event log")
    p_px_savings.add_argument("--limit", type=int, default=None, help="Only consider the last N rows")
    p_px_savings.add_argument("--json", action="store_true", help="Emit JSON output")
